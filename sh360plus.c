 ...
 * CSC 360, Summer 2018
  * Description:
 *   This adds the feature to use '->->' in place of '->' when using the OR command
 *   The double arrow means to append instead of overwrite
 */
...
void or_handler(char *commands[3][9], char *paths[], bool append){
  // copy linux command
  char lcommand[20];
  ...
  if (WIFEXITED(status) && WEXITSTATUS(status) == 12)
    fprintf(stderr, "Location of '%s' was not found on paths in .sh360\n", lcommand);
...
int main(void) {
  ...
  while(1) {
    ...
    //check for too many double arrows
    if(double_arrow_count > 0) {
      if (strcmp(type, "PP") == 0){
        printf("PP does not support '->->'\n");
        continue;
      }
    ...
    if(arrow_count == 0) {
      if (strcmp(type, "PP") == 0 || (strcmp(type, "OR") == 0 && double_arrow_count == 0)){
    ...
    if(strcmp(type, "OR") == 0) {
      or_handler(commands, paths, (double_arrow_count==1));
      continue;
    }
